...
class Gnn_net(nn.Module):
    ...
    def __call__(self, obs: GraphsTuple, n_agents: int, *args, **kwargs) -> Action:
        x = self.base_cls()(obs, node_type=0, n_type=n_agents)  #(batch_graphs, n_agents, out_dim)
        return x
...
class GnnFeatureExtractor(nn.Module):
    """
    A standard Flax module that wraps the GNN feature extractor.
    It takes a graph and extracts features for the agent nodes in the graph.
    """
    gnn_layers: int = 1
    ...
    def __call__(self, obs: GraphsTuple, n_agents: int):
        # Configure the GNN with ft.partial
        gnn_base = ft.partial(
            GNN,
            msg_dim=128,
            hid_size_msg=(256, 256),
...
class ActorWithGNN(nn.Module):
    """
    Policy network (Actor) that uses a GNN as the feature extractor.
    """
    action_dim: int
    n_agents: int # needed to call the GNN even in the single-agent case
    ...
    def __call__(self, obs: GraphsTuple):
        ...
        # 2. Feed features into the MLP head
        x = features
...
class CriticWithGNN(nn.Module):
    """
    Value network (Critic) that uses its own GNN as the feature extractor.
    """
    n_agents: int
    hidden_dims: Sequence[int] = (256, 256)
    activation: Callable = nn.relu
    ...
    def __call__(self, obs: GraphsTuple, actions: jnp.ndarray):
        ...
        # 1. Instantiate and call a dedicated GNN feature extractor
        extractor = GnnFeatureExtractor(gnn_layers=1, out_dim=128)
        # state_features shape: (n_agents, 128)
        state_features = extractor(obs, self.n_agents)
        # Path 2: process the actions only
        action_path = nn.Dense(128)(actions)
        action_path = self.activation(action_path)

        # Fuse the two paths at an intermediate layer
        x = jnp.concatenate([state_features, action_path], axis=-1)

        # 3. Feed the concatenated features into the MLP head
        for hidden_dim in self.hidden_dims:
            x = nn.Dense(hidden_dim)(x)
            x = self.activation(x)

        # 4. Output the Q value
