...
from .obstacle import Obstacle, Rectangle, Circle, GeneralObstacle
...
class Second_order(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.collision_penalty = r_c_params['collision_penalty']
        ...
        #self.create_obstacles = jax.vmap(GeneralObstacle.create) 
    ...
    def step(
            self, graph: EnvGraphsTuple, raw_action: Action, get_eval_info: bool = False
            ...
            delta_action = raw_action
            u_ref = self.u_ref(graph)

            action = delta_action + u_ref
            ...
            is_reach = dist2tgt < 0.05
            is_open = pre_min_dist2obs > 0.45
            ...
            is_success = jnp.logical_and(dist2tgt<0.05, is_timeout)
            ...
            reach_reward = jnp.where(all_reach, self.reach_reward, 0.0) 

 
            assert cost.shape == tuple()
            assert done.shape == tuple()

            correction_cost = -self.w_delta1 * (jnp.linalg.norm(raw_action, axis=-1)**2)
            
            warning_dist = self._params["car_radius"] * 4 
            danger_penalty = - (jnp.maximum(0, warning_dist - min_dist_to_obs) *self.danger_penalty_coeff )
            min_dist_to_obs = jnp.minimum(0.5, min_dist_to_obs)
            pre_min_dist2obs = jnp.minimum(0.5, pre_min_dist2obs)
        

            

            tgt_reward = (pre_dist2tgt - dist2tgt) * self.tgt_reward_coeff #
            tgt_reward = jnp.clip(tgt_reward, -0.5, 4)
    ...
    def get_cost(self, graph: EnvGraphsTuple,any_collision) -> Cost:
        ...
        cost = jnp.where(min_dist_to_obs < self.cost_dist, self.cost, cost)
    ...
    def edge_blocks(self, state: EnvState, lidar_data: State) -> list[EdgeBlock]:
        ...
        for i in range(self.num_agents):
            ...
            active_lidar = jnp.less(lidar_dist, self._params["comm_radius"] - 1e-2)
    ...
    def get_graph(self, state: EnvState, adjacency: Array = None) -> GraphsTuple:
        ...
        distances_all_beams = jnp.linalg.norm(all_lidar_data - agent_pos_expanded, axis=-1)
        min_dists = jnp.min(distances_all_beams, axis=1)
        edge_mask = jnp.where(min_dists<0.15, 1, 0)
        state_with_dist = state._replace(min_dist2obs=min_dists, edge_mask=edge_mask)
        # create graph
        graph = GetGraph(
            nodes=node_feats,
            node_type=node_type,
