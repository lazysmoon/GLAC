...
from .obstacle import Obstacle, Rectangle, Cuboid, Sphere, GeneralObstacle
...
def get_lidar(start_point: Pos, obstacles: Obstacle, num_beams: int, sense_range: float, max_returns: int = 32):
    # if isinstance(obstacles, Rectangle) or isinstance(obstacles, GeneralObstacle):
...
def get_node_goal_rng(
    ...
    def non_valid_goal(reset_input: Tuple[int, Array, Array, Array, Array]):
        ...
        dist_to_start = jnp.linalg.norm(goal - agent)
        too_short = dist_to_start <= (side_length * 0.6) #0.8 1.0

        valid = (~collide & ~inside & ~outside & ~too_long) | (i_iter >= max_iter) #& ~too_short
        out = ~valid
